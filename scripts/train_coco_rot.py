...
from training.COCO_rot import COCO_rot
...
def main(exp_name,
    ...
    print("\nStarting experiment - [train_coco_rot.py] -  `%s` @ %s\n" % (exp_name, datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
    ...
    from misc.general_utils import get_rotation_coco # get_coco_loaders
    ds_train = get_rotation_coco(image_resolution=image_resolution, model_name=model_name,
                                phase="train", test_mode=False,)
    ds_val = get_rotation_coco(image_resolution=image_resolution, model_name=model_name,
                                phase="val", test_mode=False) # test_mode should not be false here

    train = COCO_rot(
        exp_name=exp_name,
        pose_ds_train=ds_train,
        pose_ds_val=ds_val,
        aux_ds_train=None,
        aux_ds_val=None,
...
if __name__ == '__main__':
    ...
    parser.add_argument("--lr_decay_steps", help="learning rate decay steps", type=str, default="(90, 120)")
    ...
    parser.add_argument("--aux_loss", help="aux_loss", type=str, default='BCE')
    parser.add_argument("--aux_loss_weight", help="aux_loss_weight", type=float, default=0.1)
