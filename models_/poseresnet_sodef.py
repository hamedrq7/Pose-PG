...
class PoseResNet_SODEF(nn.Module):
    def __init__(self, resnet_size=50, nof_joints=17, bn_momentum=0.1):
        super(PoseResNet_SODEF, self).__init__()
        ...
        self.odefunc = ODEfunc_time_invariant(256)
        self.ode_layer = ODEBlock(self.odefunc)
        # self.before_fc = nn.Sequential(norm(256), nn.ReLU(inplace=True))
    ...
    def forward(self, x, return_feats = False):
        ...
        ode_feats = self.ode_layer(feats)
        # x = self.before_fc(x)
        x = self.final_layer(ode_feats)

        if return_feats: 
            return feats, ode_feats, x
        else: 
            return x

    def phase3_forward(self, x): 
        self.conv1.eval()
        self.bn1.eval()
        self.layer1.eval()
        self.layer2.eval()
...
if __name__ == '__main__':
    ...
    model = PoseResNet_SODEF(50, 17, 0.1)
    # print(model)
